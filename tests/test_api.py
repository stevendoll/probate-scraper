...
# ---------------------------------------------------------------------------
# Bootstrap — set env vars and mock boto3 BEFORE importing the module so the
# module-level `dynamodb = boto3.resource(...)` doesn't hit real AWS.
# Disable Powertools Tracer (avoids X-Ray daemon connection in tests).
# ---------------------------------------------------------------------------
os.environ.update(
    {
...
# Tracer requires aws-xray-sdk even when disabled — mock it entirely so
# tests have no external dependency on the X-Ray daemon or SDK.
_mock_tracer = MagicMock()
_mock_tracer.capture_lambda_handler = lambda f: f  # pass-through decorator
_mock_tracer.capture_method = lambda f: f           # pass-through decorator
...
# ---------------------------------------------------------------------------
def _call(qs=None):
    """Invoke the Lambda handler with an API Gateway-style event."""
    event = {
        "httpMethod": "GET",
        "path": "/real-estate/probate-leads/collin-tx/leads",
...
class TestHandlerFullScan(unittest.TestCase):
    """Full table scan — triggered by passing to_date="" to clear the default."""

    def setUp(self):
        self.mock_table = MagicMock()
        app.table = self.mock_table
        self.mock_table.scan.return_value = {"Items": MOCK_LEADS}

    def test_returns_200(self):
        resp = _call({"to_date": ""})
        self.assertEqual(resp["statusCode"], 200)

    def test_uses_scan_not_query(self):
        _call({"to_date": ""})
    ...
    def test_includes_warning_header(self):
        resp = _call({"to_date": ""})
        all_headers = {**resp.get("headers", {}), **resp.get("multiValueHeaders", {})}
        self.assertIn("X-Warning", all_headers)

    def test_returns_all_mock_leads(self):
        resp = _call({"to_date": ""})
        self.assertEqual(_body(resp)["count"], len(MOCK_LEADS))

    def test_no_params_defaults_to_to_date_only_query(self):
        """With no params, to_date defaults to today — so it takes the GSI path, not scan."""
        self.mock_table.query.return_value = {"Items": MOCK_LEADS[:2]}
        resp = _call({})
        self.assertEqual(resp["statusCode"], 200)
        self.mock_table.query.assert_called_once()
...
class TestHandlerDateValidation(unittest.TestCase):
    ...
    def test_nonsense_date_returns_400(self):
        resp = _call({"from_date": "not-a-date", "to_date": "2026-02-20"})
        self.assertEqual(resp["statusCode"], 400)
...
class TestHandlerErrors(unittest.TestCase):
    ...
    def test_dynamo_scan_exception_returns_500(self):
        self.mock_table.scan.side_effect = Exception("DynamoDB unavailable")
        resp = _call({"to_date": ""})  # empty to_date forces scan path
        self.assertEqual(resp["statusCode"], 500)
        self.assertIn("error", _body(resp))
...
class TestHelpers(unittest.TestCase):
    ...
    def test_encode_decode_key_roundtrip(self):
        original = {"doc_number": "123", "recorded_date": "2026-01-01", "doc_type": "PROBATE"}
