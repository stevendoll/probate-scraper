...
def _to_dynamo_item(record: dict, scrape_run_id: str) -> dict:
    """
...
def write_records(records: list, table_name: str, scrape_run_id: str) -> int:
    """
    Write *records* to *table_name* in DynamoDB.
    ...
    put_requests = [
        {"PutRequest": {"Item": _to_dynamo_item(r, scrape_run_id)}}
        for r in records
    ...
    log.info("write_records: %d/%d items written to %s", total_written, len(put_requests), table_name)
